...
- [ ] shelf (calibre-web): import commented out on nas and `shelf@home` in endpoints (2026-09-21), calibre-web 0.6.27 on nixos-unstable ships a broken entry point (`calibreweb.__main__:main`, module does not exist) so the unit crash-loops. Fixed on nixpkgs master (`calibreweb:main` in postPatch), re-enable after a flake update once nixos-unstable has it, or override `services.calibre-web.package` with master's postPatch.
