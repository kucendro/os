...
class Tree:
    ...
    def add_file(self, rel: str):
        ...
        return node.files.setdefault(
            parts[-1], {"label": parts[-1], "svcs": [], "progs": []}
        )
    ...
    def emit(self, out, indent=0):
        ...
        for fname, meta in sorted(self.files.items()):
            ...
            for s in sorted(meta["svcs"]):
                out.append(
                    f'{pad}  svc_{sanitize(s)}: "{s}" {{ shape: oval; style.fill: "#e6f0ff" }}'
                )
            for p in sorted(meta["progs"]):
                out.append(
                    f'{pad}  prog_{sanitize(p)}: "{p}" {{ shape: hexagon; style.fill: "#eaffea" }}'
                )
            out.append(f"{pad}}}")
...
def generate_modules(hosts, out_dir: Path):
    ...
    for host in hosts:
        out.append(
            f'{sanitize(host)}: "{host}" {{ shape: cloud; style.fill: "#fff3cd"; style.bold: true }}'
        )
    out.append("")
    tree.emit(out)
    out.append("")
