#!/usr/bin/env python3
"""
Auto-derive a data-flow *topology* diagram of the fleet from the Nix config.

Everything is read out of the evaluated config -- there are no host names,
domains, IP addresses or service tables baked into this script, so adding a
host, a proxied app or a mesh member needs no edits here:

  * hosts       -- attrNames of nixos/darwinConfigurations
  * addresses   -- headscale base_domain + the acl policy `hosts` map, giving
                   a derived fqdn/IP -> host book used to resolve every edge
  * mesh        -- who runs tailscale, which host runs headscale (control)
  * proxy       -- nginx vhosts + their upstream host:port (public vs tailnet
                   inferred from listenAddresses, not from any domain string)
  * monitoring  -- beszel agents -> hub, prometheus scrape/probe targets
  * subnet      -- tailscale --advertise-routes

The only service *types* named below (headscale/nginx/tailscale/prometheus/
beszel/grafana) are the mechanisms whose relationships we draw; every instance,
host and route is discovered dynamically. Output: docs/topology.d2 (+ .svg).
...
from nixdiag import (  # noqa: E402
    REPO, discover_hosts, nix_eval, sanitize, write_and_render, parse_out_flag,
)

# Per-host projection. Every attr is guarded with `or` so it works on any host
# (a darwin box has no services.nginx, a laptop no headscale, etc.).
NIXOS_PROJECT = (
    'c: { '
...
# count of enabled services, abort-free (reads per-definition values, not the
# merged .value which aborts on some renamed nixpkgs option aliases)
COUNT_SERVICES = (
...
def gather(hosts):
    ...
    for host, meta in hosts.items():
        ...
        if meta["kind"] == "nixos":
            ...
            f["svc_count"] = nix_eval(
                f".#{meta['prefix']}.{host}.options.services", COUNT_SERVICES) or 0
        else:
...
def build_address_book(hosts, facts):
    """Derived fqdn/IP -> host id. From the headscale base_domain (every host's
    tailnet name is <host>.<base>) and the acl policy `hosts` map."""
    base = ""
    policy_path = ""
...
def generate_topology(hosts, out_dir: Path):
    ...
    # which host serves each nginx vhost fqdn (so probe targets resolve)
    vhost_host = {}
    ...
    def resolve(hostport, local_host):
        ...
        if host in vhost_host:            # an fqdn served by one of our proxies
    ...
    # --- infra role nodes (read straight from each host's config) --------
    for host in hosts:
        f = facts[host]
        if f.get("headscale"):
            node(host, "headscale", "headscale\n(mesh control)", "infra")
        if any(resolve_upstream(v.get("pass"), v.get("extra")) for v in f.get("vhosts", [])):
    ...
    # --- reverse-proxy routes -> app node on the upstream host -----------
    for host in hosts:
        f = facts[host]
        local_infra = {}
        if f.get("headscale"):
        ...
        for vh in f.get("vhosts", []):
            ...
            sub = vh["name"].split(".")[0]
            # public when the vhost binds all interfaces; tailnet-only when it
            # binds a specific (tailnet) address
        ...
        for fl in facts[host].get("routes", []):
            ...
            if m:
                ...
                for net in m.group(1).split(","):
                    edges.append((f"{sanitize(host)}.subnet_router", "lan",
                                  f"advertise {net}", "#27893f"))

    # --- monitoring: beszel agents -> hub -------------------------------
    for host in hosts:
        f = facts[host]
        agent = f.get("beszelAgent") or any("beszel" in d for d in f.get("daemons", []))
        if agent and hub_host and host != hub_host:
            edges.append((sanitize(host), f"{sanitize(hub_host)}.beszel_hub",
                          "metrics", "#888"))

    # --- prometheus scrape / probe targets ------------------------------
    for host in hosts:
        f = facts[host]
        ...
        for t in f.get("promTargets", []):
            ...
            if th and th != host and th not in seen:
                seen.add(th)
                edges.append((f"{sanitize(host)}.prometheus", sanitize(th),
                              "scrape / probe", "#888"))
